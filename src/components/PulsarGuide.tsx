...
function getContextualGuide(pathname: string, lang: 'fr' | 'en'): GuideStep {
  ...
  // ── CASCADE ──
  if (pathname.match(/\/patient\/[^/]+\/cascade/)) {
    return {
      message: "Le Cascade Alert Engine détecte les effets en chaîne AVANT qu'ils ne se produisent. Testez un médicament dans le champ ci-dessus — PULSAR croise avec le profil de vulnérabilité du patient et la littérature (OpenFDA, ANSM).",
      messageEn: "The Cascade Alert Engine detects chain effects BEFORE they happen. Test a drug in the field above — PULSAR cross-references with the patient's vulnerability profile and literature (OpenFDA, ANSM).",
      tip: "Né du cas Alejandro : le MEOPA administré à un enfant en prodrome FIRES a déclenché la cascade convulsive. PULSAR aurait dit STOP.",
      tipEn: "Born from Alejandro's case: MEOPA administered to a child in FIRES prodrome triggered the seizure cascade. PULSAR would have said STOP.",
    }
  }

  // ── DDD ──
  if (pathname.match(/\/patient\/[^/]+\/ddd/)) {
    return {
  ...
  if (pathname.match(/\/patient\/[^/]+\/consult/)) {
    return {
      message: "Un clic → le brief expert complet. 8 sections + questions auto-générées avec niveau d'urgence. Basculez FR/EN et copiez pour envoyer au spécialiste.",
      messageEn: "One click → the complete expert brief. 8 sections + auto-generated questions with urgency level. Toggle FR/EN and copy to send to the specialist.",
      action: { label: 'Copier', labelEn: 'Copy', href: '#copy' },
    }
  }

  // ── DISCOVERY ──
  if (pathname.match(/\/patient\/[^/]+\/discovery/)) {
    return {
      message: "Le Discovery Engine croise les données de CE patient avec la recherche mondiale. 4 niveaux : corrélations statistiques, PubMed temps réel, hypothèses IA, essais cliniques ClinicalTrials.gov.",
      messageEn: "The Discovery Engine cross-references THIS patient's data with global research. 4 levels: statistical correlations, real-time PubMed, AI hypotheses, ClinicalTrials.gov trials.",
      tip: "Les termes de recherche sont auto-construits depuis le diagnostic du patient.",
      tipEn: "Search terms are auto-built from the patient's diagnosis.",
    }
  }

  // ── RAPPORT ──
  if (pathname.match(/\/patient\/[^/]+\/rapport/)) {
    return {
      message: "Rapport complet généré automatiquement par les 11 moteurs. Cliquez 'Exporter PDF' pour sauvegarder ou imprimer.",
      messageEn: "Complete report auto-generated by all 11 engines. Click 'Export PDF' to save or print.",
      action: { label: 'Exporter PDF', labelEn: 'Export PDF', href: '#export' },
    }
  }

  // ── FEEDBACK ──
  if (pathname.match(/\/patient\/[^/]+\/feedback/)) {
    return {
      message: "Le FeedbackLoop capture des snapshots anonymisés à chaque checkpoint. Chaque enfant qui entre dans PULSAR rend le système plus intelligent pour le suivant.",
      messageEn: "FeedbackLoop captures anonymized snapshots at each checkpoint. Every child who enters PULSAR makes the system smarter for the next one.",
      tip: "Patient 0 (Alejandro) a généré 8 learnings qui enrichissent automatiquement les futurs diagnostics FIRES.",
      tipEn: "Patient 0 (Alejandro) generated 8 learnings that automatically enrich future FIRES diagnoses.",
    }
  }
